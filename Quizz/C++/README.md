...
空指针是一种特殊的指针,它处于空闲状态, 并将其初始化为空指针, 初始化后p的值为0, 不指向任何地址, 当判断指针是否为空指针时, 需要空指针与空指针常量NULL作比较.

野指针不是空指针, 而是指向不明或不当的地址, 野指针很难识别, 有时会隐藏的很深, 野指针非常危险, 可能发生不可预知的错误, 野指针常见的三种情况

1. 指针未初始化: 指针变量在初始化的时候不会自动初始为空指针, 而是一个随机的值, 可能指向任意空间,
2. 指针指向的变量被free或delete后没有设置为NULL, 在调用free或delete释放空间后, 指针指向的内容被销毁, 空间被释放, 但指针的值未改变仍指向这块内存, 这就使得指针称为野指针, 在调用free或delete后需将指针设置为NULL
3. 指针操作超过所指向变量的生命周期: 当指针指向变量的生命周期已经结束时, 指针仍指向这块空间, 就使得该指针称为野指针.
